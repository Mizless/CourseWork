...
class Field:
    ...
    def black_score(self) -> int:
        '''Счёт чёрных'''
        return sum(reduce(lambda acc, checker: acc + (checker.type == CheckerType.BLACK_REGULAR) + (checker.type == CheckerType.BLACK_QUEEN) * 3 + (checker.type == CheckerType.BLACK_KING) * 100, checkers, 0) for checkers in self.__checkers)