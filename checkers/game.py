...
class Game:
    ...
    def __initialization_images(self):
        ...
        self.__images = {
            ...
            CheckerType.WHITE_KING: ImageTk.PhotoImage(
                Image.open(Path('assets', 'white-king.png')).resize((CELL_SIZE, CELL_SIZE), Image.ANTIALIAS)),
            CheckerType.BLACK_KING: ImageTk.PhotoImage(
                Image.open(Path('assets', 'black-king.png')).resize((CELL_SIZE, CELL_SIZE), Image.ANTIALIAS))
        }
    ...
    def __animate_move(self, move: Move):
        ...
        animated_checker = self.__canvas.create_image(move.from_x * CELL_SIZE, move.from_y * CELL_SIZE, image=self.__images.get(self.__field.type_piece(move.from_x, move.from_y)), anchor='nw', tag='animated_checker')
    ...
    def __draw_field_grid(self):
        ...
        for y in range(self.__field.y_size):
            for x in range(self.__field.x_size):
                self.__canvas.create_rectangle(x * CELL_SIZE, y * CELL_SIZE, x * CELL_SIZE + CELL_SIZE, y * CELL_SIZE + CELL_SIZE, fill=FIELD_COLORS[(y + 1 + x) % 2], width=0, tag='boards')
    ...
    def __draw_checkers(self):
        ...
        for y in range(self.__field.y_size):
            for x in range(self.__field.x_size):
                ...
                if (self.__field.type_piece(x, y) != CheckerType.NONE and not (x == self.__animated_cell.x and y == self.__animated_cell.y)):
                    self.__canvas.create_image(x * CELL_SIZE, y * CELL_SIZE, image=self.__images.get(self.__field.type_piece(x, y)), anchor='nw', tag='checkers')
    ...
    def __handle_move(self, move: Move, draw: bool = True) -> bool:
        ...
        elif (move.to_y == self.__field.y_size - 1 and self.__field.type_piece(move.from_x, move.from_y) == CheckerType.BLACK_REGULAR):
            ...
            for x in range(self.__field.x_size):
                ...
                if (self.__field.type_piece(x, y) == friendly_checkers[0]):
                    for offset in MOVE_OFFSETS:
                        ...
                        if self.__field.type_piece(x + offset.x, y + offset.y) in enemy_checkers and self.__field.type_piece(x + offset.x * 2, y + offset.y * 2) == CheckerType.NONE:
                ...
                # Для дамки
                    ...
                    for offset in MOVE_OFFSETS:
                        ...
                        for shift in range(1, self.__field.size):
                            if not (self.__field.is_within_in_field(x + offset.x * shift, y + offset.y * shift)): continue

                            # Если на пути не было вражеской шашки
                            if (not has_enemy_checker_on_way):
                                if (self.__field.type_piece(x + offset.x * shift, y + offset.y * shift) in enemy_checkers):
                                    has_enemy_checker_on_way = True
                                    continue
                                # Если на пути союзная шашка - то закончить цикл
                                elif (self.__field.type_piece(x + offset.x * shift, y + offset.y * shift) in friendly_checkers):
                                    break
                            
                            # Если на пути была вражеская шашка
                            if (has_enemy_checker_on_way):
                                if (self.__field.type_piece(x + offset.x * shift, y + offset.y * shift) == CheckerType.NONE):
